...
def RunPSZScan(config, particle):
	...
	# for i_z in range(1765, 2006, 10):
		...
		ut.Plot2D(df['P'], df['y'], 500, 0, 500, 500, -250, 250, title, "Momentum [MeV]", "y [mm]", "../img/"+g4blVer+"/PSZScan/h2_YvsMom_"+ntupleName+"_"+particle+"_"+config+".png", cb=False)

		ut.Plot2D(df['PT'], df['x'], 500, 0, 500, 250, -250, 250, title, "Tranverse momentum [MeV]", "x [mm]", "../img/"+g4blVer+"/PSZScan/h2_XvsMomT_"+ntupleName+"_"+particle+"_"+config+".png", cb=False)
		ut.Plot2D(df['PT'], df['y'], 500, 0, 500, 500, -250, 250, title, "Tranverse momentum [MeV]", "y [mm]", "../img/"+g4blVer+"/PSZScan/h2_YvsMomT_"+ntupleName+"_"+particle+"_"+config+".png", cb=False)

		ut.Plot2D(df['Pz'], df['x'], 1000, -500, 500, 250, -250, 250, title, "Longitundinal momentum [MeV]", "x [mm]", "../img/"+g4blVer+"/PSZScan/h2_XvsMomZ_"+ntupleName+"_"+particle+"_"+config+".png", cb=False)
		ut.Plot2D(df['Pz'], df['y'], 1000, -500, 500, 500, -250, 250, title, "Longitundinal momentum [MeV]", "y [mm]", "../img/"+g4blVer+"/PSZScan/h2_YvsMomZ_"+ntupleName+"_"+particle+"_"+config+".png", cb=False)

		# ut.Plot1D(df['Theta'], int(np.pi)*1000, -np.pi/2, np.pi/2, title, r"$\theta$ [rad]", "Counts / mrad", fout="../img/"+g4blVer+"/PSZScan/h1_Theta_"+ntupleName+"_"+particle+"_"+config+".png", legPos="best", stats=False, errors=False)
		# ut.Plot1D(df['Phi'], int(np.pi)*1000, -np.pi/2, np.pi/2, title, r"$\phi$ [rad]", "Counts / mrad", fout="../img/"+g4blVer+"/PSZScan/h1_Phi_"+ntupleName+"_"+particle+"_"+config+".png", legPos="best", stats=False, errors=False)

		# # Plot XY at each z
		ut.Plot1D(df['x'], 400, -200, 200, title, "x [mm]", "Counts / mm", "../img/"+g4blVer+"/PSZScan/h1_X_"+ntupleName+"_"+particle+"_"+config+".png", stats=False) # , cb=False)
		ut.Plot1D(df['y'], 400, -200, 200, title, "y [mm]", "Counts / mm", "../img/"+g4blVer+"/PSZScan/h1_Y_"+ntupleName+"_"+particle+"_"+config+".png", stats=False) # , cb=False)
		...
		# ut.Plot2D(df['PT'], df['Theta'], 500, 0, 500, int(np.pi)*1000, -np.pi/2, np.pi/2, title, "$P_{T}$ [MeV]", r"$\theta$ [rad]", "../img/"+g4blVer+"/PSZScan/h2_ThetaVsPT_"+ntupleName+"_"+particle+"_"+config+".png", cb=False)
	...
	# ut.PlotGraph(x=z_, y=[x * 1000 for x in alpha_], xerr=[], yerr=[], title=particle, xlabel="z [mm]", ylabel=r"$\langle \alpha \rangle$ [mrad]", fout="../img/"+g4blVer+"/PSZScan/gr_AvgAlphaVsZ_"+particle+"_"+config+".png")
	# ut.PlotGraph(x=z_, y=[x * 1000 for x in beta_], xerr=[], yerr=[], title=particle, xlabel="z [mm]", ylabel=r"$\langle \beta \rangle$ [mrad]", fout="../img/"+g4blVer+"/PSZScan/gr_AvgBetaVsZ_"+particle+"_"+config+".png")
